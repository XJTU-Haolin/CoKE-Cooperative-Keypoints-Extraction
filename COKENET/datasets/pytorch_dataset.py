...
import numpy as np
import COKENET.datasets.dataset_utils as tools
from tqdm import tqdm
from torch.utils.data import Dataset
from COKENET.aux.tools import check_directory
...
class pytorch_dataset(Dataset):
    def __init__(self, data, mode='train'):
        ...
        if mode == 'train':
            if len(self.data) > 9000: #9000
                self.data = self.data[:9000]
        elif mode == 'val':
            if len(self.data) > 3000:  #3000
...
class DatasetGeneration(object):
    ...
    def _create_pair_images(self, is_val):
        ...
        for path_image_idx in tqdm(range(len(self.images_info))):
            ...
            if is_val and counter_patches > 9000: #9000
                break
            elif counter_patches > 3000:  #3000
                break
            ...
            # if is_val and counter_patches > 2:
            #     break
            # elif counter_patches > 2:
            #     break
            # if is_val and self.is_debugging and counter_patches > 100:
            #     break
            # elif not is_val and self.is_debugging and counter_patches > 400:
