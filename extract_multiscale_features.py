...
def extract_features(image, image_orin, ratio, model1, model2, device, levels, point_level, args, chief_features, mode):
    ...
    for (j, resized) in enumerate(pyramid):
        ...
        if mode =='chief_infer':
            network, im_scores,network_follower,output_follower,output_next,output_feat_follower,o1,o2 = model1(im,im,mode=mode,chief_features=chief_features)
            output_next_all.append(output_next)
        elif mode == 'follower_infer':
            network, output_chief,network_follower,im_scores,output_feat_chief,output_next,o1,o2 = model1(im, im, mode=mode, chief_features=chief_features[j])
            output_next_all.append(output_next)
    ...
    if args.upsampled_levels:
        for k in range(args.upsampled_levels):
            ...
            if mode == 'chief_infer':
                network, im_scores, network_follower, output_follower, output_next, output_feat_follower,o1,o2 = model1(im,
                                                                                                                  im,
                                                                                                                  mode=mode,
                ...
                network, output_chief, network_follower, im_scores, output_feat_chief, output_next,o1,o2 = model1(im, im,
                                                                                                            mode=mode,
                                                                                                            chief_features=chief_features[j+k+1])
                output_next_all.append(output_next)
            im_scores = F.relu(im_scores)
            im_scores = geo_tools.remove_borders(im_scores[0,0,:,:].cpu().detach().numpy(), borders=args.border_size)
    ...
    return im_pts, descriptors, output_next_all


def extract_COKE_features():
    args = get_config()
    ...
    for path_to_image in iterate:
        # seq_name =
        path = args.data_dir + path_to_image.rstrip('\n')
        ...
        if cur_seq_name != seq_name:
            ...
            with torch.no_grad():
                ...
                # ratio = [1.0,1.0]
                im_pts, descriptors,chief_features = extract_features(im_resize,im,ratio, model1, model2, device, levels, point_level, args, chief_features, 'chief_infer' )
                # im_pts, descriptors, chief_features = extract_features(im, im, ratio, model1, model2, device,
                #                                                        levels, point_level, args, chief_features,
                #
                #                                                        i'chief_infer')
                if cur_id == '1':
                    cur_seq_chief_features = chief_features
        else:
            orin_size = [im.shape[0], im.shape[1]]
            with torch.no_grad():
                im_re = im.copy()
                ...
                ratio = [orin_size[1]/im_resize.shape[1], orin_size[0]/im_resize.shape[0] ]
                im_pts, descriptors, chief_features = extract_features(im_resize, im,ratio, model1, model2, device, levels, point_level,args, cur_seq_chief_features, 'follower_infer')

        torch.cuda.empty_cache()
        file_name = os.path.join(args.results_dir, version_network_name, path_to_image.rstrip('\n'))+'.kpt'
