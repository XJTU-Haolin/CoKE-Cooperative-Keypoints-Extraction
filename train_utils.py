...
## Loss function.
from COKENET.loss.score_loss_function import msip_loss_function
## validation.
...
def training_epochs(epoch, dataloader, model, kernels, optimizer, MSIP_sizes, MSIP_factor_loss,  weight_coordinates, patch_size, device):
    ...
    tic = time.time()
    iterate = tqdm(enumerate(dataloader), total=len(dataloader), desc="COKE Training")
    for idx, batch in iterate:
        ...
        network1, output1, network2, output2,output_feat_chief,output_feat_follower,output3,output4 = model(images_src_batch,images_dst_batch)


        src_score_maps = F.relu(output1)
        dst_score_maps = F.relu(output2)

        src_score_maps_orin = F.relu(output3)
        dst_score_maps_orin = F.relu(output4)
        ...
        for MSIP_idx in range(len(MSIP_sizes)):
            MSIP_loss_no_att, loss_elements = msip_loss_function(images_src_batch.permute(0,3,1,2), src_score_maps_orin,  dst_score_maps_orin,
                                        MSIP_sizes[MSIP_idx], kernels, h_src_2_dst_batch, h_dst_2_src_batch,
                                        weight_coordinates, patch_size, input_border_mask
                                        )
            MSIP_level_name = "MSIP_ws_{}_orin".format(MSIP_sizes[MSIP_idx])
            MSIP_elements[MSIP_level_name] = loss_elements

            loss1 += MSIP_factor_loss[MSIP_idx] * MSIP_loss_no_att
        ...
        for MSIP_idx in range(len(MSIP_sizes)):
            ...
            loss2 += MSIP_factor_loss[MSIP_idx] * MSIP_loss
            # print("MSIP_level_name {} of MSIP_idx {} : {}, {} ".format(MSIP_level_name, MSIP_idx,  MSIP_loss, MSIP_factor_loss[MSIP_idx] * MSIP_loss)) ## logging
        loss = 0.25*loss1 + 0.75*loss2  #0.5/0.5   2023-2-15:0.8/0.2
        total_loss_avg.append(loss)
        iterate.set_description("current loss : {:0.4f}, avg loss : {:0.4f}, loss1:{:0.4f}, loss2:{:0.4f}".format(loss, torch.stack(total_loss_avg).mean(), loss1, loss2 ))
        ...
        if idx % 50 == 0:
            ...
            cv2.imwrite('COKENET/data/image_dst_' + post_fix + '.png', 255 * images_dst_batch[0,:,:,0].cpu().detach().numpy())  ## Tensorflow style input (B,H,W,C)
            cv2.imwrite('COKENET/data/COKENET_dst_' + post_fix + '.png', 255 * deep_dst[0,0,:,:] / deep_dst[0,0,:,:].max())
            cv2.imwrite('COKENET/data/image_src_' + post_fix + '.png', 255 * images_src_batch[0,:,:,0].cpu().detach().numpy())
            cv2.imwrite('COKENET/data/COKENET_src_' + post_fix + '.png', 255 * deep_src[0,0,:,:] / deep_src[0,0,:,:].max())
...
def check_val_rep(dataloader, model, nms_size, device,  num_points=25):
    ...
    error_overlap_m = []
    possible_matches = []
    iterate = tqdm(enumerate(dataloader), total=len(dataloader), desc="COKENET Validation")
    ...
    for _, batch in iterate:
        ...
        # network1, output1 = model(images_src_batch)
        # network2, output2 = model(images_dst_batch)
        network1,output1,network2,output2,output_feat_chief,output_feat_follower,o1,o2 = model(images_src_batch,images_dst_batch)
        src_score_maps = F.relu(output1)
        dst_score_maps = F.relu(output2)
        ...
        iterate.set_description("COKENET Validation time {:0.3f} {:0.3f} {:0.3f} {:0.3f} {:0.3f}".format(ba, cb, dc, ed, fe ))
