...
import COKENET.config as config
import COKENET.aux.tools as aux
from train_utils import training_epochs, check_val_rep, fix_randseed

## Load data
from COKENET.datasets.pytorch_dataset import DatasetGeneration, pytorch_dataset
from torch.utils.data import DataLoader
## Network architecture & loss / optimizer
# from COKENET.model.keynet_architecture import keynet
from COKENET.model.coke_architecture import iminet
import torch.optim as optim
## Training loop
from tqdm import tqdm
import time
from COKENET.aux.logger import Logger

args = config.get_config()
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

# Check directories
aux.check_directory('COKENET/data')
...
print('Start training ImiNet Architecture')
...
model = iminet(args, device, MSIP_sizes)
...
if args.resume_training != '':
    ...
    for name, value in model.named_parameters():
        if 'UNet3D' not in name:
            value.requires_grad = True

    # print(model.learner.conv_0.weight)
    # print(model.last_layer_learner.conv_last.weight)
...
'''
with torch.no_grad():
    iminet_rep_val,_,_,_,_ = check_val_rep(dataloader_val, model, args.nms_size, device, num_points=25)
    best_rep_s = iminet_rep_val
    best_epoch = -1
    logging.info(('\n Epoch -1 : Repeatability Validation: {:.3f}.'.format(iminet_rep_val)))
'''
