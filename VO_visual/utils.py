...
def drawFrameFeatures(frame, prevPts, currPts, frameIdx):
    ...
    for i in range(len(currPts)-1):
        cv2.circle(currFrameRGB, tuple(np.int64(currPts[i])), radius=3, color=(200, 100, 0))
        cv2.line(currFrameRGB, tuple(np.int64(prevPts[i])), tuple(np.int64(currPts[i])), color=(200, 100, 0))
...
def updateTrajectoryDrawing(trackedPoints, groundtruthPoints):
    plt.cla()
    plt.plot(trackedPoints[:,0], trackedPoints[:,2], c='blue', label="oFAST(ORB)")
...
def updateTrajectoryDrawing1(trackedPoints, groundtruthPoints,trackedPoints1,trackedPoints3):
    plt.cla()
    plt.plot(trackedPoints[:,0], trackedPoints[:,2], c='blue', label="oFAST(ORB)")
    plt.plot(trackedPoints1[:, 0], trackedPoints1[:, 2], c='black', linestyle=':', label="Key.Net")
    plt.plot(trackedPoints3[:, 0], trackedPoints3[:, 2], c='red', label="CoKE")
    plt.plot(groundtruthPoints[:,0], groundtruthPoints[:,2], c='green', label="Ground Truth")
    plt.title("Trajectory")
    plt.xlabel("X [m]")
    plt.ylabel("Y [m]")
