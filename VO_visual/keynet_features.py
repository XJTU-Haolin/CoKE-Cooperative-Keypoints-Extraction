...
from COKENET.config_hpatches import get_config
import COKENET.aux.tools as aux
from COKENET.keynet.keynet_architecture import keynet
from COKENET.keynet.hardnet_pytorch import HardNet
## Network architecture
import COKENET.aux.desc_aux_function as loss_desc
import HSequences_bench.tools.geometry_tools as geo_tools
import HSequences_bench.tools.repeatability_tools as rep_tools
...
def keynet_detect(model1,index,chief_features,imagesPath):
    ...
    with torch.no_grad():
        ...
        ratio =[1.0,1.0]
        im_pts, _ = extract_keypoint(im, model1, device, levels, point_level, args,ratio)

    return im_pts


def extract_keypoint(image, model1, device, levels, point_level, args,ratio):
...
def keynet_create():
    ...
    #
    model_debug = keynet(args, device, MSIP_sizes)
    # model_debug.load_state_dict(
    #     torch.load('/data0/ZHL/project/keynet/COKE/COKENET/pretrained_nets/keyNet.pt'))
    model_debug.load_state_dict(
        torch.load('COKENET/pretrained_nets/ab/bs_2_8.pt'))

    # COKENET / pretrained_nets / ab / bs_2_8.pt
